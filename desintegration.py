...
def slha_to_dataframe_mod2(file_path, particle_type):
    ...
    decay_datas = []
    for particle_id in particle_ids:
         decay_datas.append(data.decays[particle_id])

    _ = 0 
    for decay_data in decay_datas:
          particle_id = particle_ids[_]
          for decay in decay_data.decays:
               df_data.append({'decay_products' : tuple(sorted(decay.ids)), 'branching_ratio' : decay.br, 'particle_id' : particle_id})
          _+=1
    df = pd.DataFrame(df_data, columns = ['decay_products', 'branching_ratio', 'particle_id'])
...
def plot_branching_ratios_on_ax2(ax_neutralino, ax_chargino, neutralino_df, chargino_df, title_neutralino, title_chargino):
    # Neutralinos
     ...
     for neut,ax in zip([1000023, 1000025, 1000035], list(ax_neutralino)):

          neutralino_df_part = neutralino_df[neutralino_df['particle_id'] == neut]

          decay_products_neutralino = neutralino_df_part['decay_products']
          num_decay_products_neutralino = len(decay_products_neutralino)
          bar_positions_neutralino = np.arange(num_decay_products_neutralino)
          for i, decay_product in enumerate(decay_products_neutralino):
               ax.bar(bar_positions_neutralino[i], neutralino_df_part[neutralino_df_part['decay_products'] == decay_product]['branching_ratio'].mean(), label=str(decay_product))

          ax.set_xticks(bar_positions_neutralino)
          ax.set_xticklabels(decay_products_neutralino, rotation=45, ha='right')
          ax.set_ylabel('Branching Ratio')
          ax.set_title(title_neutralino)
          ax.legend()

    # Charginos
     for charg,ax in zip([1000024, 1000037], list(ax_chargino)):
          chargino_df_part = chargino_df[chargino_df['particle_id'] == charg]
          decay_products_chargino = chargino_df_part['decay_products']
          num_decay_products_chargino = len(decay_products_chargino)
          bar_positions_chargino = np.arange(num_decay_products_chargino)

          for i, decay_product in enumerate(decay_products_chargino):
               ax.bar(bar_positions_chargino[i], chargino_df[chargino_df['decay_products'] == decay_product]['branching_ratio'].mean(), label=str(decay_product))
          ax.set_xticks(bar_positions_chargino)
          ax.set_xticklabels(decay_products_chargino, rotation=45, ha='right')
          ax.set_ylabel('Branching Ratio')
          ax.set_title(title_chargino)
          ax.legend()
...
def update_graph(m1_value, m2_value, mu_value, tan_beta_value):
    ...
    (nmix, umix,vmix, neutralino_df, chargino_df) = load_data_from_files(m1_value, m2_value, mu_value, tan_beta_value)
    ...
    plot_mixing_matrix_on_ax(ax2, umix, "Chargino U Mixing Matrix")
    plot_mixing_matrix_on_ax(ax3, vmix, "Chargino V Mixing Matrix")
    
    plot_branching_ratios_on_ax2((ax4,ax5, ax6), (ax7,ax8), neutralino_df, chargino_df, "Neutralino Decay Branching Ratios", "Chargino Decay Branching Ratios")
    plt.tight_layout()
...
def load_data_from_files(m1_value, m2_value, mu_value, tan_beta_value):
     ...
     neutralino_df = slha_to_dataframe_mod2(filename, particle_type = 'neutralino')
     chargino_df = slha_to_dataframe_mod2(filename, particle_type = 'chargino')

     _ = slha_to_dataframe_mod2(filename, particle_type = 'neutralino')
     nmix = np.zeros((4,4))
     umix = np.zeros((2,2))
     vmix = np.zeros((2,2))
     for i in range(1,5):
     ...
     for i in range(1,3):
          for j in range(1,3):
               if (i,j) in data.blocks['VMIX']:
                    vmix[i-1,j-1] = data.blocks['VMIX'][i,j]

     return nmix, umix,vmix, neutralino_df, chargino_df
